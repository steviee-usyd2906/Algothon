...
TARGET_PORTFOLIO_DAILY_VOL  = np.nan  # now calibrated via VOL_TARGET_GRID instead of fixed
# ----------------------------- signal config ------------------------------
...
DEFAULT_VOL_TARGET = 4_000.0   # pre-calibration fallback for the 5th swept dim below

# --- sweep grid for the five thresholds above ------------------------------
# v2: KELLY_GRID widened past 1.0 -- diagnostics on the un-fixed sweep showed
# it was picking KELLY_FRACTION=0.25 (the *lowest* grid point) purely because
# the old objective (Sharpe) doesn't reward the extra dollar P&L that a bigger,
# still-risk-capped book would produce (see _score below). Same reasoning for
# widening DD_CAP_GRID / CVAR_BUDGET_GRID: at higher kelly the fixed-dollar
# caps bind sooner, so they need more headroom to still be useful.
# TARGET_PORTFOLIO_DAILY_VOL is now VOL_TARGET_GRID -- a 5th swept dimension --
# instead of a hand-fixed constant, since it directly sets the leverage
# ceiling of the whole book.
KELLY_GRID = [0.25, 0.5, 0.75, 1.0, 1.5, 2.0]
DD_CAP_GRID = [3_000.0, 6_000.0, 10_000.0, 15_000.0]
DD_WINDOW_GRID = [30, 60, 90]
CVAR_BUDGET_GRID = [3_000.0, 5_000.0, 8_000.0, 12_000.0]
VOL_TARGET_GRID = [2_000.0, 4_000.0, 8_000.0, 15_000.0]
...
SWEEP_DD_PENALTY = 0.0        # optional extra drawdown regularizer, see _score(); off by default
                               # because the real score() below already penalises variance
SCORE_PARAM = 1.0              # must match eval.py's scoreDefaultParam
VERBOSE_CALIBRATION = True     # print what the sweep picked, and on how much data
...
COV_MIN_HISTORY = 20
# NOTE: portfolio vol target itself is now calibrated -- see VOL_TARGET_GRID /
# DEFAULT_VOL_TARGET above and params["VOL_TARGET"] below -- rather than fixed.

# --- commission rates (must match eval.py so the sweep prices turnover the
# same way the real grader does; otherwise calibration can favour high-
# turnover combos that look good gross but bleed out net of fees) ----------
COMM_RATE_DEFAULT = 0.0001
COMM_RATE_INST0 = 0.00002
...
def _commission_rates(nInst):
    rates = np.full(nInst, COMM_RATE_DEFAULT, dtype=float)
    if nInst > 0:
        rates[0] = COMM_RATE_INST0
    return rates
...
def _precompute_for_sweep(train_prices):
    ...
    return {
        ...
        "comm_rates": _commission_rates(nInst),
    }


def _simulate_combo(pre, kelly, dd_cap, dd_window, cvar_budget, vol_target):
    """Cheap per-combo pass: reuses everything precomputed above, only
    re-running the parts that actually depend on the five swept values
    (drawdown scale trajectory, Kelly haircut, vol target, CVaR cap).
    Also charges the same per-instrument commission eval.py charges, on
    the day-over-day change in dollar_position, so a combo that only looks
    good gross (high turnover chasing noise) doesn't win the sweep."""
    days = pre["days"]
    Td = len(days)
    ...
    comm_rates = pre["comm_rates"]

    trend_scale = 1.0
    pnl_series = np.zeros(Td)
    prev_dollar_position = np.zeros_like(trend_dollar_full[0])
    ...
    for i in range(Td):
        ...
        Sigma = sigma_list[i]
        if Sigma is not None and np.isfinite(vol_target):
            port_var = dollar_position @ Sigma @ dollar_position
            port_vol = np.sqrt(max(port_var, 0.0))
            if port_vol > 1e-9:
                dollar_position = dollar_position * min(1.0, vol_target / port_vol)
        ...
        gross_pnl = float(shares @ (next_prices[i] - prices[i]))
        turnover_dollars = np.abs(dollar_position - prev_dollar_position)
        commission = float(np.sum(turnover_dollars * comm_rates))
        pnl_series[i] = gross_pnl - commission
        prev_dollar_position = dollar_position

    return pnl_series


def _score(pnl_series):
    """v2: this now mirrors eval.py's own score(mu, sigma, param) formula
    exactly (same sqrt(250), same param=1.0), instead of a bare Sharpe.

    This matters because eval.py's score is NOT scale-invariant the way
    Sharpe is: for a fixed Sharpe ratio, score scales ~linearly with the
    size of the book (scale mu and sigma up together by k and score scales
    by k too). The old Sharpe-only objective couldn't see that, so it had
    no reason to prefer a bigger, still-risk-capped book over a smaller
    one with a marginally better ratio -- diagnostics on this codebase
    showed it was picking KELLY_FRACTION=0.25, the most conservative point
    in the old grid, purely because it nudged the ratio up a little, while
    leaving most of the position-limit/CVaR/drawdown-capped headroom the
    live risk overlays are already there to protect completely unused.
    Optimizing the real formula directly fixes that, using the same
    risk caps as guardrails rather than the optimization target.
    """
    mu = float(pnl_series.mean())
    sigma = float(pnl_series.std())
    if mu <= 0 or sigma < 1e-10:
        base = mu
    else:
        sr = np.sqrt(250) * mu / sigma
        frac = sr ** 2 / (sr ** 2 + SCORE_PARAM ** 2)
        base = mu * frac
    if SWEEP_DD_PENALTY > 0:
